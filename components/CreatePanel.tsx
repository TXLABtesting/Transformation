    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, direction: 'rtl' }}>
      ...
      >
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: 24 }}>
          ...
          {m.mStep === 'review' && <ReviewStep vm={vm} />}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {optCard(
        ...
        IC.pencil,
        'التعبئة اليدوية',
        'املأ النموذج بنفسك مع إمكانية الاستعانة بالذكاء الاصطناعي في مراجعة الحقول قبل الإرسال.'
      )}
      {optCard(
        () => s.chooseBulk(),
        '#E3F6EC',
        ...
        <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <input
              ...
              style={inputStyle}
            />
            <span style={{ fontSize: 13, color: '#9AA6BC' }}>نموذج</span>
          </div>
        </div>
        <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            <div>
              ...
              <input
                type="date"
                value={ph.start || ''}
                onChange={(e) => s.updPhaseDate(pi, 'start', e.target.value)}
                style={inputStyle}
              />
            </div>
            <div>
              ...
              <input
                type="date"
                value={ph.end || ''}
                onChange={(e) => s.updPhaseDate(pi, 'end', e.target.value)}
                style={inputStyle}
              />
            </div>
          </div>
...
function ReviewStep({ vm }: { vm: VM }) {
  ...
  const section = (title: string, items: string[], icon: string, color: string) => (
    <div style={{ ...cardStyle }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
        <Icon d={icon} size={16} color={color} />
        <span style={{ fontSize: 13, fontWeight: 800, color: '#1F2D49' }}>{title}</span>
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {items.map((it, i) => (
          <div key={i} style={{ display: 'flex', gap: 8, fontSize: 12.5, color: '#54627B', lineHeight: 1.7 }}>
            <span style={{ width: 5, height: 5, borderRadius: '50%', background: color, marginTop: 7, flex: 'none' }} />
            <span>{it}</span>
          </div>
        ))}
      </div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 11, marginBottom: 16 }}>
        ...
        >
          <Icon d={IC.sparkle} size={20} color="#fff" fill="#fff" />
        </div>
        <div>
          <div style={{ fontSize: 15, fontWeight: 800, color: '#13213C' }}>المراجعة الذكية</div>
          <div style={{ fontSize: 11.5, color: '#9AA6BC' }}>
            تتحقق من اكتمال المحتوى ووضوحه وصحة تصنيفه قبل الإرسال
          </div>
        </div>
      </div>
      ...
      {m.aiLoading && (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: '40px 0' }}>
          ...
          <div style={{ fontSize: 13, color: '#54627B', fontWeight: 700 }}>
            جارٍ مراجعة المحتوى بالذكاء الاصطناعي…
          </div>
        </div>
      )}
      ...
      {!m.aiLoading && r && (
        <>
          <div style={{ display: 'flex', gap: 10, marginBottom: 16 }}>
            {tile(m.aiReadyCount, 'جاهز', '#0B8A4B', '#E3F6EC')}
            {tile(m.aiImproveCount, 'بحاجة لتحسين', '#B45309', '#FFF3DE')}
            {tile(m.aiNotesCount, 'ملاحظات مهمة', '#D23B45', '#FCEEEF')}
          </div>

          {section('جاهز للإرسال', r.ready, IC.check, '#0B8A4B')}
          {section('بحاجة إلى تحسين', r.improve, IC.warnTri, '#B45309')}
          {section('ملاحظات مهمة', r.notes, IC.infoCircle, '#D23B45')}
...
function BulkStep({ vm }: { vm: VM }) {
  ...
  return (
    <div>
      ...
      >
        استخدم القالب المرفق فقط. بعد الرفع تُراجَع الجاهزية بالذكاء الاصطناعي قبل الاعتماد.
      </div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 11, marginBottom: 16 }}>
        ...
        <div>
          <div style={{ fontSize: 15, fontWeight: 800, color: '#13213C' }}>المراجعة الذكية للعناصر المرفوعة</div>
          <div style={{ fontSize: 11.5, color: '#9AA6BC' }}>يتم فحص كل عنصر قبل الإرسال للاعتماد</div>
        </div>
      </div>
